...
from pyfcm.algorithms import Hard
...
class TestHard(TestCase):
    ...
    def test_membership_calculation_range(self, _, clusters, features):
        ...
        self.assertTrue(np.all((clusterer.memberships == 1) | (clusterer.memberships == 0)))
    ...
    def test_fit_initialization_count(self, _, n_init):
        ...
        clusterer = Hard(2, n_init=n_init)
    ...
    def test_converge_update_count(self, _, n_iter):
        data, labels = make_blobs()
        clusterer = Hard(2, max_iter=n_iter)
